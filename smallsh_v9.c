...
{
    /* Open or create the file specified */
    int outfp = open(output, O_WRONLY | O_CREAT | O_TRUNC, 0644);


    /* If there was an error, print out an error and exit (status 1) */
    if (outfp == -1)
    {
    ...
    /* Using dup2(), redirect output */
    int outdup = dup2(outfp, 1);
...
void redirect_input(char * input) 
{
    ...
    /* Use dup2() to redirect the input */
    int indup = dup2(infp, 1);

    /* Handle any errors in dup2() */
    if(indup == -1)
    {
        printf("Dup2 Error!\n");
        fflush(stdout);
 ...
 * @param shellpid 
 */
void background_process(char ** arguments, int argc, int shellpid) // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - TO DO - - - |||
{
    /* local variables */
...
void prep(char * line, char ** argv, int argc, int shellpid, bool try_bg)
{
    ...
    if ((try_bg) && (!tstp))
    {
        background_process(argv, argc, shellpid); 
    }
